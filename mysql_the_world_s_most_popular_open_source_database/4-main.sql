...
SELECT TVShow.name as 'TVShow name', Episode.name as 'Episode name' FROM TVShow JOIN Season ON (TVShow.id = Season.tvshow_id) JOIN Episode ON (Season.id = Episode.season_id) WHERE Episode.number = 1 AND Season.number = 1;

\! echo "\nList of all Genres by all TVShows ordered by TVShow name (A-Z)? (if a genre has 0 TVShow, please display NULL as TVShow name)"
SELECT TVShow.name as 'TVShow name', Genre.name as 'Genre name' FROM TVShow RIGHT JOIN TVShowGenre ON (TVShow.id = TVShowGenre.tvshow_id) RIGHT JOIN Genre ON (TVShowGenre.genre_id = Genre.id) ORDER BY TVShow.name;
